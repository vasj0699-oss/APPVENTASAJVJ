...
export default function Usuarios() {
  ...
  const [form, setForm] = useState({ email: "", password: "", name: "", role: "capturista" });
  ...
  const create = async () => {
    ...
    try {
      ...
      setForm({ email: "", password: "", name: "", role: "capturista" });
      setShow(false);
      load();
    } catch (err) {
  ...
  if (!me) return <div className="text-[#4d5e42]">Cargando…</div>;
    ...
    <div className="space-y-6" data-testid="usuarios-page">
      ...
      {show && (
        <div className="card-surface p-5 grid grid-cols-1 md:grid-cols-4 gap-3">
          <div>
            <label className="text-xs uppercase tracking-widest font-semibold text-[#4d5e42]">Nombre</label>
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
              data-testid="user-name-input" className="mt-1 w-full border border-[#deedc0] rounded-md bg-white px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-xs uppercase tracking-widest font-semibold text-[#4d5e42]">Email</label>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
              data-testid="user-email-input" className="mt-1 w-full border border-[#deedc0] rounded-md bg-white px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-xs uppercase tracking-widest font-semibold text-[#4d5e42]">Contraseña</label>
            <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
              data-testid="user-password-input" className="mt-1 w-full border border-[#deedc0] rounded-md bg-white px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-xs uppercase tracking-widest font-semibold text-[#4d5e42]">Rol</label>
      ...
      <div className="card-surface overflow-hidden">
        {loading ? (
          <div className="p-6 text-sm text-[#4d5e42]">Cargando…</div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {users.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-[#4d5e42]">No hay usuarios</td></tr>
              )}
            </tbody>
          </table>
        )}

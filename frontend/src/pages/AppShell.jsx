...
export default function AppShell() {
  ...
  const navItems = [
    ...
    { to: "/modulos", icon: Boxes, label: "Por Módulo", testid: "nav-modulos", adminOnly: false },
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#deedc0] px-2 py-2 flex gap-1 overflow-x-auto">
          {navItems.filter((n) => !n.adminOnly || isAdmin).map((n) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex-shrink-0 flex flex-col items-center gap-1 px-3 py-1.5 rounded text-[10px] ${
                  isActive ? "bg-[#2d4a12] text-white" : "text-[#4d5e42]"
                }`
              }
            >
              <n.icon className="w-4 h-4" strokeWidth={1.8} />
